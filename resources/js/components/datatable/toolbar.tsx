...
export default function TableToolbar<TData, TValue>({ table }: TableToolbarProps<TData, TValue>) {
   return (
      <div className={'flex items-center justify-between gap-2 px-0'}>
         <div className="right-section flex items-center ">
            <TextInput
               iconPosition={'start'}
               icon={<IconSearch size={16} stroke={1.5} className={'text-slate-600'} />}
               placeholder={'Cari...'}
         ...
         <div className="left-section">
            <DropdownMenu modal={false}>
               <DropdownMenuTrigger asChild>
                  <Button variant="outline" className="ml-auto z-0 w-auto select-none focus-visible:ring-0 gap-1">
                     <IconTableOptions size={14} stroke={1.5} />
                     <span className={'hidden md:block'}>Kolom</span>
                  </Button>
               </DropdownMenuTrigger>
               <DropdownMenuContent className={'p-0 rounded-md w-auto shadow-sm border-slate-100'} align="end">
                  <div className="dropdown-header p-3 border-b border-slate-100">
                     <h5 className={'text-sm font-medium px-2 capitalize'}>Pilih Untuk Ditampilkan</h5>
                  </div>
                  <div className={'p-2'}>
                     <DropdownMenuCheckboxItem

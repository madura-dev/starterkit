...
const FileInput = forwardRef<HTMLInputElement, FileInputProps>(function FileInput(props, ref) {
   ...
   const thumbs = files.map(file => (
      <div key={file.name} className={'flex gap-2 mt-2 '}>
         <div className={'max-w-40 group max-h-40 rounded-md overflow-hidden relative'}>
            <img
               src={file.preview}
               className={'min-h-40 min-w-40 object-cover rounded-md border-2 p-2 border-slate-100 bg-slate-100/20'}
               onLoad={() => {
                     ...
                     <Fragment>
                        <div className="grid gap-2 w-full text-center justify-center">
                           <div className="flex items-center justify-center">
                              <IconCloudUpload size={48} stroke={1.25} className={'text-primary text-center'} />
                           </div>
                           <h5 className={'font-semibold'}>
                              <span className={'text-primary'}>Pilih File</span> or Drag File
                           </h5>
                        </div>
                     </Fragment>
                  )}

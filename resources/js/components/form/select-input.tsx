...
const SelectInput: React.FC<SelectInputProps> & React.InputHTMLAttributes<HTMLInputElement> = ({
   children,
   variant = 'simple',
   options = [],
   onSelect,
   ...
   useEffect(() => {
      if (props.value) {
         const selected: string | any = options.find(option => option.value == props.value)?.label;
         setValue(selected);
      }
   }, []);
               ...
               <Command>
                  ...
                  <CommandGroup>
                     <CommandList className={'w-full'}>
                        {options.map(option => (
                           <CommandItem key={option.value} value={option.value} onSelect={value => handleSelect(value)}>
                              {option.label}
                              <CheckIcon
